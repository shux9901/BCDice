...
MESSAGETEXT
  ...
  TABLES = {
    'FT' => SparseTable.new(
      ...
      [
        [ 3, '【余裕】が3点減少する（最低0まで）'],
        [ 5, 'ランダムな荷物1個が落ちて行方不明になる（大失敗したエリアのアイテム調査で見つけることが可能）'],
        [ 7, 'ランダムな荷物1個が破壊される'],
        [ 9, 'ランダム天気表を使用し、結果をターンの終了まで適用する'],
    ...
    'ROMT' => SparseTable.new(
      'ランダム遮蔽物表(屋外)',
      '1D12',
      [
        [ 2, '【藪】耐久度3,軽減値1,特殊効果:コンタクト内のキャラクターに対する射撃攻撃判定に-1の修正を付加'],
        [ 5, '【木】耐久度5,軽減値2,特殊効果:コンタクト内のキャラクターに対する射撃攻撃判定に-1の修正を付加'],
        [ 8, '【大木】耐久度7,軽減値3,特殊効果:コンタクト内のキャラクターに対する射撃攻撃判定に-2の修正を付加'],
        [10, '【岩】耐久度6,軽減値4,特殊効果:コンタクト内のキャラクターに対する射撃攻撃判定に-1の修正を付加/コンタクト内で行われる格闘攻撃のダメージ+1'],
        [12, '【岩壁】耐久度8,軽減値4,特殊効果:コンタクト内のキャラクターに対する射撃攻撃判定に-2の修正を付加/コンタクト内で行われる格闘攻撃のダメージ+2'],
      ]
    ),
    'RIMT' => SparseTable.new(
      'ランダム遮蔽物表(屋内)',
      '1D12',
      [
        [ 4, '【木材の壁】耐久度4,軽減値2,特殊効果:コンタクト内のキャラクターに対する射撃攻撃判定に-1の修正を付加'],
        [ 8, '【木材の扉】耐久度4,軽減値2,特殊効果:コンタクト内のキャラクターに対する射撃攻撃判定に-1、接触判定と突撃判定に-2の修正を付加'],
        [12, '【木製家具】耐久度3,軽減値2,特殊効果:コンタクト内で行われる格闘攻撃のダメージ+1'],
      ]
    ),
    'EET' => SparseTable.new(
      '逃走体験表',
      ...
      [
        [ 3, '【余裕】が0になる'],
        [ 6, '任意の【絆】を合計2点減少する'],
        [ 9, '全ての荷物を失う（逃走したエリアに配置され、調査で発見可能）'],
        [12, '全ての武器と防具と小道具と荷物を失う（逃走したエリアに配置され、調査で発見可能）'],
      ]
    ),
    'GFT' => SparseTable.new(
      '食材採集表',
